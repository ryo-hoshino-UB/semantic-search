...
export default async function Page({ searchParams }: PageProps) {
  ...
  // queryによるヒット件数が3件未満の場合、ベクトル検索を行う
  if (products.length < 3) {
    ...
    // ベクトル検索でヒットした商品のうち、既存の商品と重複していないものを追加する
    const vectorProducts = res
      .filter((existingProduct) => {
        if (
          // product=query検索でヒットした商品、existingProduct=ベクトル検索でヒットした商品
    ...
    return (
      <div className="text-center py-4 bg-white shadow-md rounded-b-md">
        ...
        <p className="mt-1 text-sm mx-auto max-w-prose text-gray-500">
          ...
          <span className="text-green-500 font-medium">{query}</span>.
        </p>
      </div>
    );
  }
        ...
        <Link key={product.id} href={`/products/${product.id}`}>
          <li className="mx-auto py-4 px-8 flex space-x-4">
            ...
            <div className="w-full flex-1 space-y-2 py-1">
              ...
              <p className="prose prose-sm text-gray-900 line-clamp-3">
                {product.description}
              </p>
